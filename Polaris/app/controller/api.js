...
module.exports = {
  ...
  register: function *() {
    ...
    try {
      ...
      var tokenNo = yield tokenModel.putToken(loginInfo, passportUser);
      var result = {access_token:tokenNo, user:passportUser,msg:'登录成功'};
      var p2pUserId = yield userModel.findUserByPassportUser(passportUser.id);
      if(inviteCode) { //如果填写了邀请码
        tclog.notice({msg:"triggerInteract register", user:p2pUserId, inviteCode:inviteCode});
        interactModel.triggerInteract(0, p2pUserId, inviteCode, p2pUserId);
      }
      yield this.api(result);
    } catch (err) {
